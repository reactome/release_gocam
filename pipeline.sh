...
# Enable debugging and redirect to stdout
PS4='+ '  # Customize the prompt for better readability
set -x
...
java -jar -Xmx15G pathways2GO/exchange/bin/biopax2go.jar -b $BIOPAX_PATH/Homo_sapiens.owl -bg $REACTO_OUT/blazegraph.jnl -o $REACTO_OUT/ -e REACTO -dc GOC:reactome_curators -dp https://reactome.org -lego $TARGET_PATH/go-lego.owl > $REPORTS_PATH/biopax2go.log
grep DELETING_NON_SMALL_MOL_REGULATOR $REPORTS_PATH/biopax2go.log | cut -f2- > $REPORTS_PATH/deleted_regulators.tsv
java -jar -Xmx15G pathways2GO/exchange/bin/biopax2go.jar -b $BIOPAX_PATH/Homo_sapiens.owl -reacto $TARGET_PATH/reacto -chebi $TARGET_PATH/chebi.owl -lego $TARGET_PATH/go-lego.owl
...
java -Xmx15G -cp pathways2GO/exchange/bin/biopax2go.jar org.geneontology.garage.Manuscript -b $BIOPAX_PATH/Homo_sapiens.owl -j $REACTO_OUT/reacto-models-bg.jnl -g $REACTO_OUT/ -r $REPORTS_PATH/
